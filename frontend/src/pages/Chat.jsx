    ...
    <div className="flex flex-col h-[calc(100vh-8rem)]">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center pb-4 border-b border-slate-100 gap-4">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1">Ask questions and search context inside your documents.</p>
        </div>
        ...
        <div className="flex items-center gap-2 w-full sm:w-auto">
          ...
          <select
            ...
            className="bg-white border border-slate-200 rounded-xl px-4 py-2 text-slate-800 focus:outline-none focus:border-indigo-500 text-sm w-full sm:w-64"
          >
      ...
      <div className="flex-1 overflow-y-auto py-6 space-y-6">
        {messages.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full text-center max-w-md mx-auto space-y-4">
            ...
            <h3 className="text-lg font-bold text-slate-800">Ask your Study Material</h3>
            <p className="text-sm text-slate-400">Select a textbook or lecture notes file above to ask targeted questions. Answers are fully cited from the text.</p>
          </div>
        ) : (
          messages.map((msg, i) => (
            <div 
              ...
              <div className="flex-1 space-y-3">
                ...
                {msg.role === 'assistant' && msg.citations && msg.citations.length > 0 && (
                  <button 
                    ...
                    className="flex items-center gap-1.5 text-xs font-bold text-indigo-600 hover:text-indigo-700 transition-all bg-indigo-50 hover:bg-indigo-100/50 px-3 py-1.5 rounded-xl border border-indigo-100 cursor-pointer"
                  >
      ...
      <form onSubmit={handleSend} className="pt-4 border-t border-slate-100 flex gap-3">
        <input
          ...
          className="flex-1 bg-white border border-slate-200 rounded-xl px-5 py-4 text-slate-800 focus:outline-none focus:border-indigo-500 text-sm placeholder-slate-400 disabled:opacity-50"
        />
        <button
          type="submit"
          disabled={!selectedDocId || loading || !input.trim()}
          className="bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl px-6 flex items-center justify-center transition-all cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed shadow-lg shadow-indigo-100"
        >
      ...
      {showCitationModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm">
          <div className="bg-white border border-slate-200 max-w-2xl w-full rounded-2xl overflow-hidden shadow-2xl">
            <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50">
              ...
              <button 
                ...
                className="text-slate-500 hover:text-slate-950 font-bold text-sm cursor-pointer"
              >
                Close
              </button>
            </div>
              ...
              {citations.map((cit, idx) => (
                <div key={idx} className="p-4 bg-slate-50 border border-slate-100 rounded-xl space-y-2">
                  <div className="flex justify-between items-center text-xs font-bold text-slate-400">
                    ...
                    <span className="bg-slate-250 text-slate-600 px-2 py-0.5 rounded">Page {cit.page || cit.page_number}</span>
                  </div>
                  <p className="text-sm text-slate-650 leading-relaxed italic">"...{cit.content}..."</p>
                </div>
              ))}
            </div>
          </div>
